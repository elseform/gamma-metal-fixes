...
numbers must move (for example, heat at the scope centre 0.0 → 1.0). The measurements are recorded with the validation evidence.
...
измениться (например, тепло в центре прицела 0.0 → 1.0). Результаты сохраняются вместе с материалами проверки.
