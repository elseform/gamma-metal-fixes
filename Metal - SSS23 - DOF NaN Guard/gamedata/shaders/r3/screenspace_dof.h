...
float3 SSFX_DOF(float2 tc, float3 depth, float3 img)
{
	...
	// [elseform] Clamp smoothstep bounds to prevent edge0 == edge1 division by zero under Metal/DXMT.
	float dof_min = ssfx_wpn_dof_1.x;
	float dof_max = max(ssfx_wpn_dof_1.y, dof_min + 1e-4f);
	float blur_w = lerp(ssfx_wpn_dof_1.w, 0.0f, smoothstep(dof_min, dof_max, depth.z));	
	...
	if (ssfx_wpn_dof_2 > 0)
	{
		...
		// [elseform] Guard smoothstep upper bound against 0 divisor under Metal.
		float edge_max = max(1e-4f, saturate(1.0f - ssfx_wpn_dof_2));
 		edgeBlur = pow(smoothstep(0.0f, edge_max, length(mid_uv)), 1.5f) * 1.33f;
	...
	if (ssfx_wpn_dof_1.z > 0)
	{
		...
		// [elseform] Guard depth length against negative or zero.
		float d_len = length(depth);
		img = lerp(img, blur_far, saturate(smoothstep(1.0f, 2.4f, d_len) + int(depth.z <= SKY_EPS)) * ssfx_wpn_dof_1.z);
	}
