...
float3 SSFX_ssr_water_ray(float3 ray_start_vs, float3 ray_dir_vs, float noise, out float HitDepth, float2 tc, uint iSample : SV_SAMPLEINDEX)
{
	...
	// [elseform] Bound quality index lookup.
	int water_q = clamp(SSFX_WATER_QUALITY, 0, 4);
	int max_water_steps = clamp(q_steps[water_q].x, 1, 64);
	int bin_search_steps = clamp(q_steps[water_q].y, 1, 8);

	float RayThick = clamp( 48.0f / max_water_steps, 1.0f, 3.0f);

	// Initialize Ray
	RayTrace ssr_ray = SSFX_ray_init(ray_start_vs, ray_dir_vs, 150, max_water_steps, noise);
	...
	[loop]
	for (int st = 1; st <= max_water_steps; st++)
	{
		// Ray out of screen...
		...
		if (ray_check.x > 0)
		{
			...
			// Binary Search
			[loop]
			for (int x = 0; x < bin_search_steps; x++)
			{
		...
		bool PTh = (!NoWpnSky && ray_check.y > 0.01f && st > max_water_steps * 0.4f);
