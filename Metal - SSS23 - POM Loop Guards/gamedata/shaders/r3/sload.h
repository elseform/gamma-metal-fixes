...
*/
	void UpdateTC( inout p_bumped I, inout float H )
	{
		...
		// [elseform] Metal loop guard: hard iteration cap. Termination normally
		// relies on curr_step growing past curr_Height; a degenerate _step or
		// NaN feed can otherwise spin the loop and hang the GPU under
		// Metal/DXMT (no TDR recovery on macOS).
		int _guard = 0;
		...
		} while(curr_Height >= curr_step && ++_guard < 128);
		...
		// [ Contact Refinement ]
			...
			_guard = 0; // [elseform] Metal loop guard (see above)
			...
			} while(curr_Height >= curr_step && ++_guard < 64);
...
surface_bumped sload_i( p_bumped I )
{
	...
	S.gloss		= Nu.x; // ES Style
#else
	S.gloss		= Nu.x * Nu.x; // Vanilla Style
#endif
