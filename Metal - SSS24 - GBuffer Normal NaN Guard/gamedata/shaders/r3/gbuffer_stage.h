...
struct f_deffer
{
	...
	float4 H : SV_Target2; // HeatVision
	float4 velocity : SV_Target3; // Motion vectors
...
float3 gbuf_unpack_normal( float2 norm )
{
   ...
   // [elseform] Protect sqrt domain from precision overshoot.
   res.y = ( norm.y < 0 ? -1.0 : 1.0 ) * sqrt( max(0.0, 1 - res.x * res.x - res.z * res.z ) );
...
float3 gbuf_unpack_normal( float2 norm )
{
    ...
    // [elseform] Metal Second Pass: feather phi.x to 0 over the last ~6% of the
    // unit-sphere domain instead of a hard sqrt(max(0.0, ...)) clamp -- same fix shape
    // as Metal - 3DSS5 - Lens NaN Guard's sample_lens_normalmap(). The hard clamp
    // matched the limit value (0) but not its slope, producing a visible ring/banding
    // artifact wherever a curved specular surface's normal sweeps through this pole.
    float d2 = norm.y * norm.y;
    float edge_fade = smoothstep(0.0, 0.06, 1.0 - d2);
    float phi_x = sqrt(max(0.0, 1.0 - d2) * edge_fade);
    float2 phi = float2(phi_x, norm.y);
...
gbuffer_data gbuffer_load_data( float2 tc : TEXCOORD, float2 pos2d, uint iSample )
{
	...
	// Unjitter position buffer to avoid shadow flickering and more related issues
    pos2d -= ssfx_jitter.xy * (float2(screen_res.x, -screen_res.y) * 0.5f);
