...
// [elseform] Under a DLSS render scale the G-buffer and heat buffer are smaller than the
// scope draw's screen_res, so pixel positions must come from the buffers themselves.
float2 thermal_buffer_size()
{
    uint w, h;
#ifdef USE_MSAA
    uint samples;
    s_heat.GetDimensions(w, h, samples);
#else
...
{	
    // [elseform] Load in the heat/G-buffer's own pixel space, not the scope pass's screen_res.
    float2 buffer_size = thermal_buffer_size();
    float2 buffer_pos = min(saturate(Tex0) * buffer_size, buffer_size - 1.0);
    HPos = buffer_pos;
    float3 hotness3 = s_heat.Load(int3(buffer_pos, 0 ), 0).rgb;
	...
	// [elseform] Initialize: lerp() below reads it even when hotness is zero.
