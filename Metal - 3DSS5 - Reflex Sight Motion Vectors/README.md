# Metal - 3DSS5 - Reflex Sight Motion Vectors
...
The glass (`models_reflex_lens`: reflections and dirt) also sits on the
weapon but wrote no motion vectors, so it was reprojected with the world
and smeared while panning; with the glass made transparent in a test
build the smudge was almost gone. Its vertex shader now passes clip
positions and jitters like the reticle, and its pixel shader writes
`(HUD motion, IsHUD = 1, alpha = glass opacity)` into the motion-vector
target. The glass keeps its stock `srcalpha/invsrcalpha` blend, which then
gives `lerp(world, HUD, opacity)` per pixel.
...
- `models_reflex_lens.vs`, `models_reflex_lens.ps`
...
Shaders 24. It supersedes the retired `Metal - 3DSS5 - Reflex Reticle Semantics`.
